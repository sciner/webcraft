...
export class ServerGame {
        ...
        if (this.shutdownPromise) {
            ...
            if (this.shutdownGentle && !gentle) {
                ...
                for (const world of this.worlds.values()) {
                    if (world.shuttingDown) {
                        // skip worlds that were created after shutdown order
    ...
    async processWorldQueue() {
        ...
        for (const world_guid of this.worlds_loading.keys()) {
            ...
            const conn = await SQLiteServerConnector.connect(
                `../world/${world_guid}/world.sqlite`,
            );
            ...
            console.log(
                'World started',
                Math.round((performance.now() - p) * 1000) / 1000 + 'ms',
            );
            break;
        }
    ...
    initWorkers() {
        return new Promise((resolve, reject) => {
            ...
            this.lightWorker = new Worker(
                globalThis.__dirname + '/../www/js/light_worker.js',
            );
    ...
    async initWs() {
        ...
        this.wsServer.on('connection', (conn, req) => {
            ...
            const onWorld = async () => {
                ...
                player.onJoin(
                    query.session_id,
                    parseFloat(query.skin),
                    conn,
                    world,
                );
                const game_world = await this.db.getWorld(world_guid);
                await this.db.IncreasePlayCount(
                    game_world.id,
                    query.session_id,
                );
            };
