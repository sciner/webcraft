    ...
    static async call(method, params, session_id, req) {
        ...
        switch (method) {
            case '/api/Game/getWorldPublicInfo':
                const world = await ServerAPI.getDb().getWorld(
                    params.worldGuid,
                );
                // mapping
                const woldPublicInfo = {
                    ...
                    cover: world.cover
                        ? `/worldcover/${world.guid}/screenshot/${world.cover}`
                        : null,
                };
            ...
            case '/api/User/Registration': {
                const session = await ServerAPI.getDb().Registration(
                    params.username,
                    params.password,
                );
            ...
            case '/api/User/Login': {
                const session = await ServerAPI.getDb().Login(
                    params.username,
                    params.password,
                );
            ...
            case '/api/Game/CreateWorld': {
                ...
                if (
                    [config.building_schemas_world_name].includes(params.title)
                ) {
                    const session = await ServerAPI.getDb().GetPlayerSession(
                        session_id,
                    );
                ...
                const generator = WorldGenerators.validateAndFixOptions(
                    params.generator,
                );
                ...
                const world = await ServerAPI.getDb().InsertNewWorld(
                    session.user_id,
                    generator,
                    seed,
                    title,
            ...
            case '/api/Game/JoinWorld': {
                ...
                const world = await ServerAPI.getDb().JoinWorld(
                    session.user_id,
                    world_guid,
                );
                Log.append('JoinWorld', {
                    user_id: session.user_id,
                    world_guid,
                });
                return world;
            ...
            case '/api/Game/DeleteWorld': {
                ...
                const resp = await ServerAPI.getDb().DeleteWorld(
                    session.user_id,
                    world_guid,
                );
                return resp;
            ...
            case '/api/Game/Screenshot': {
                ...
                if (req.files && session) {
                    const filename = await ServerAPI.getDb().InsertScreenshot(
                        world_id,
                        params.as_cover == 'true',
                    );
                    if (filename) {
                        ...
                        if (typeof fs === 'undefined') {
                            ...
                            */
                            ...
                            const path =
                                '../world/' + world_id + '/screenshot/';
                            if (!fs.existsSync(path)) {
                                fs.mkdirSync(path, { recursive: true });
                            }
                            screenshot_file.mv(path + filename);
                            screenshot_file_preview.mv(
                                path + `preview_${filename}`,
                            );
                        }
                        return { result: 'ok' };
                    }
            ...
            case '/api/Skin/Upload': {
                ...
                const params = req.body;
                const skin_id = await ServerAPI.getDb().skins.upload(
                    params.data,
                    params.name,
                    params.type,
                    session.user_id,
            ...
            case '/api/Skin/DeleteFromUser': {
                ...
                const params = req.body;
                await ServerAPI.getDb().skins.deleteFromUser(
                    session.user_id,
                    params.skin_id,
                );
                return { result: 'ok' };
