...
function checkVersion(module_name, current) {
    ...
    if (!semver.satisfies(current, need_version)) {
        console.error(
            `${module_name} required version ${need_version}, but present is ${current}`,
        );
        process.exit();
    }
}
...
Config.init().then(async (config) => {
    ...
    global.SERVER_TIME_LAG = config.Debug ? (0.5 - Math.random()) * 50000 : 0;
    global.EMULATED_PING = config.Debug ? Math.random() * 100 : 0;
    ...
    global.skiaCanvas = skiaCanvas;
    global.mkdirp = mkdirp;

    global.plugins = new PluginManager(global.config);
    ...
    app.use(
        compression({
            ...
            filter: (req, res) => {
                const ext = req._parsedUrl.pathname
                    .split('.')
                    .pop()
                    .toLowerCase();
    ...
    app.use(
        '/style',
        expressLess(__dirname + '/../www/style', {
            compress: true,
            debug: true,
        }),
    );
    ...
    app.use('/api', async (req, res) => {
        try {
            const resp = await ServerAPI.call(
                req.originalUrl,
                req.body,
    ...
    app.use('/worlds', async (req, res) => {
        ...
        res.render(pathToIndex, {
            page: { ...page, title: `${config.ProjectName} - ${world.title}` },
            world,
        });
    });
