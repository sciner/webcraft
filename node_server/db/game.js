...
export class DBGame {
    ...
    async applyMigrations() {
        ...
        try {
            ...
            await this.conn.get(
                'CREATE TABLE "options" ("id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, "version" integer NOT NULL DEFAULT 0)',
            );
        ...
        migrations.push({
            version: 3,
            queries: [`alter table user add column "flags" INTEGER DEFAULT 0`],
        });
        ...
        migrations.push({
            version: 8,
            queries: [`ALTER TABLE "world" ADD "cover" TEXT;`],
        });
        ...
        for (let m of migrations) {
            if (m.version > version) {
                ...
                await this.conn.get(
                    'update options set version = ' + ++version,
                );
                await this.conn.get('commit');
                version = m.version;
    ...
    async LogAppend(event_name, data) {
        await this.conn.run(
            'INSERT INTO log(dt, event_name, data) VALUES (:dt, :event_name, :data)',
            {
                ':dt': unixTime(),
    ...
    async ReferrerAppend(url, headers) {
        await this.conn.run(
            'INSERT INTO referrer(dt, url, headers) VALUES (:dt, :url, :headers)',
            {
    ...
    async Registration(username, password) {
        if (
            await this.conn.get(
                'SELECT id, username, guid, password FROM user WHERE username = ?',
                [username],
            )
        ) {
        ...
        const result = await this.conn.run(
            'INSERT OR IGNORE INTO user(dt, guid, username, password) VALUES (:dt, :guid, :username, :password)',
            {
                ':dt': unixTime(),
                ':guid': guid,
                ':username': username,
        ...
        if (!result.changes) {
            // If it's a single-player, or insertion failed in multi-player
            const row = await this.conn.get(
                'SELECT id AS lastID FROM user WHERE guid = :guid',
                {
                    ':guid': guid,
                },
            );
            ...
            if (lastID == 1) {
                await this.conn.run(
                    'UPDATE user SET flags = 256 WHERE _rowid_ = :id',
                    {
                        ':id': lastID,
                    },
                );
    ...
    async Login(username, password) {
        const result = await this.conn.get(
            'SELECT id, username, guid, password FROM user WHERE username = ? and password = ?',
            [username, password],
        );
        if (!result) {
    ...
    async GetPlayerSession(session_id) {
        const row = await this.conn.get(
            'SELECT u.id, u.username, u.guid, u.flags FROM user_session s LEFT JOIN user u ON u.id = s.user_id WHERE token = :session_id LIMIT 1',
    ...
    async CreatePlayerSession(user_row) {
        ...
        await this.conn.run(
            'INSERT INTO user_session(dt, user_id, token) VALUES (:dt, :user_id, :session_id)',
            {
                ':dt': unixTime(),
                ':user_id': user_row.id,
                ':session_id': session_id,
    ...
    async MyWorlds(user_id) {
        ...
        const rows = await this.conn.all(
            'SELECT w.id, w.dt, w.user_id, w.guid, w.title, w.seed, w.generator, w.cover, w.game_mode FROM world_player AS wp LEFT JOIN world w ON w.id = wp.world_id WHERE wp.user_id = :user_id ORDER BY wp.dt_last_visit DESC, wp.id DESC',
            {
                ':user_id': user_id,
            },
        );
        if (rows) {
            for (let row of rows) {
                const cover = row.cover
                    ? row.cover + (row.cover.indexOf('.') > 0 ? '' : '.webp')
                    : null;
                const cover_preview = cover
                    ? cover.startsWith('scr')
                        ? `preview_${cover}`
                        : null
                    : null;
                const world = {
    ...
    async DeleteWorld(user_id, guid) {
        const row = await this.conn.get(
            'SELECT id FROM world WHERE guid = :guid',
            {
                ':guid': guid,
            },
        );
        ...
        await this.conn.run(
            'DELETE FROM world_player WHERE user_id = :user_id AND world_id = :world_id',
            {
                ':user_id': user_id,
                ':world_id': world_id,
            },
    ...
    async InsertNewWorld(user_id, generator, seed, title, game_mode) {
        ...
        const result = await this.conn.run(
            'INSERT OR IGNORE INTO world(dt, guid, user_id, title, seed, generator, pos_spawn, game_mode) VALUES (:dt, :guid, :user_id, :title, :seed, :generator, :pos_spawn, :game_mode)',
            {
                ':dt': unixTime(),
                ':guid': guid,
                ':user_id': user_id,
        ...
        if (!result.changes) {
            // If it's a single-player, or insertion failed in multi-player
            const row = await this.conn.get(
                'SELECT id AS lastID FROM world WHERE guid = :guid',
                {
                    ':guid': guid,
                },
            );
    ...
    async InsertWorldPlayer(world_id, user_id) {
        ...
        const dt = unixTime();
        const result = await this.conn.run(
            'INSERT INTO world_player(dt, world_id, user_id, dt_last_visit) VALUES (:dt, :world_id, :user_id, :dt_last_visit)',
            {
                ':dt': dt,
                ':world_id': world_id,
                ':user_id': user_id,
        ...
        if (!result.lastID) {
            const row = await this.conn.get(
                'SELECT id AS lastID FROM world_player WHERE user_id = :user_id',
                {
                    ':user_id': user_id,
                },
            );
    ...
    async getWorldID(world_guid) {
        const row = await this.conn.get('SELECT id FROM world WHERE guid = ?', [
            world_guid,
        ]);
        if (!row) {
            throw 'error_world_not_found';
    ...
    async PlayerExistsInWorld(world_id, user_id) {
        const result = await this.conn.get(
            'SELECT id FROM world_player WHERE world_id = ? and user_id = ?',
            [world_id, user_id],
        );
        return !!result;
    }
    ...
    async InsertScreenshot(guid, cover) {
        ...
        if (cover) {
            await this.conn.run(
                'UPDATE world SET cover = :cover WHERE guid = :guid',
                {
                    ':cover': filename,
                    ':guid': guid,
                },
        ...
        await this.conn.run(
            'INSERT INTO screenshot (dt, guid_world, guid_file) VALUES (:dt, :guid, :file)',
            {
                ':dt': unixTime(),
                ':guid': guid,
                ':file': filename,
