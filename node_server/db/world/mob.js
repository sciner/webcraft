...
const BULK_UPDATE_FIELDS =
    'x = %1, y = %2, z = %3, indicators = %4, extra_data = %5, rotate = %6';
...
export class DBWorldMob {
    constructor(
        ...
        this.bulkLoadActiveInVolumeQuery = new BulkSelectQuery(
            this.conn,
    ...
    async init() {
        this.maxId =
            (
                await this.conn.get(
                    'SELECT id FROM entity ORDER BY id DESC LIMIT 1',
                )
            )?.id ?? 0;
    }
    ...
    _cacheMob(id, is_active, x = null, y = null, z = null) {
        ...
        if (new_chunk_addr) {
            this._activeMobsInChunkCount.update(
                new_chunk_addr,
                (n) => (n ?? 0) + 1,
            );
    ...
    async initChunksWithMobs() {
        ...
        let rows = await this.conn
            .all(`SELECT id, x, y, z /*, is_active DISTINCT
    ...
    static upgradeRowToFullUpdate(row, mob) {
        row.push(mob.is_active ? 1 : 0, JSON.stringify(mob.pos_spawn));
    }
    ...
    async bulkFullUpdate(rows) {
        ...
        `);
        return rows.length
            ? run(this.conn, UPDATE.BULK_FULL, {
                  ':jsonRows': JSON.stringify(rows),
              }).then(() => {
                  for (const row of rows) {
