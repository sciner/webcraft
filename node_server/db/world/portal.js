...
export class DBWorldPortal {
    ...
    async add(user_id, portal) {
        const result = await this.conn.run(
            'INSERT INTO portal(user_id, dt, x, y, z, rotate, size, player_pos, portal_block_id, type, pair) VALUES(:user_id, :dt, :x, :y, :z, :rotate, :size, :player_pos, :portal_block_id, :type, :pair)',
            {
                ':dt': unixTime(),
                ':user_id': user_id,
                ':x': portal.pos.x,
        ...
        if (!result.lastID) {
            const row = await this.conn.get(
                'SELECT _rowid_ AS lastID FROM portal WHERE x = :x AND y = :y AND z = :z ORDER by _rowid_ DESC',
                {
                    ':x': portal.pos.x,
                    ':y': portal.pos.y,
                    ':z': portal.pos.z,
    ...
    async search(pos, max_dist, type) {
        const row = await this.conn.get(
            `WITH portals AS (SELECT _rowid_ as id, *, ((x - :x) * (x - :x) + (y - :y) * (y - :y) + (z - :z) * (z - :z)) as dist
        FROM portal
    ...
    async setPortalPair(portal_id, pair) {
        await this.conn.run(
            'UPDATE portal SET pair = :pair WHERE _rowid_ = :portal_id',
            {
                ':portal_id': portal_id,
    ...
    async getByID(portal_id) {
        const row = await this.conn.get(
            `SELECT _rowid_ as id, * FROM portal
