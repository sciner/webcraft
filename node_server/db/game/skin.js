...
export class DBGameSkins {
    ...
    async updateStaticSkins() {
        ...
        for (var skin of staticSkinsById.values()) {
            ...
            const result = await this.conn.run(
                `INSERT OR IGNORE INTO skin (id, dt, file, type, rights, hash) 
            ...
            if (result.changes) {
                ...
                if (id && id !== skin.id) {
                    resp.errors.push(
                        `Skin id=${skin.id} can't be added because skin id=${id} has the same hash and type.`,
                    );
                } else {
                    // We can't insert it because it has the same id, or it's the same skin. Update it.
                    await this.conn.run(
                        'UPDATE OR IGNORE skin SET hash = :hash, file = :file, type = :type WHERE id = :id',
                        {
                            ':hash': hash,
                            ':file': skin.file,
                            ':type': skin.type,
    ...
    async getSkinByHashType(hash, type) {
        return await this.conn.get(
            'SELECT * FROM skin WHERE hash = ? AND type = ?',
            [hash, type],
        );
    }
    ...
    async addUserSkin(user_id, skin_id) {
        ...
        await this.conn.run(
            `INSERT OR IGNORE INTO user_skin (user_id, skin_id, dt) VALUES (:user_id, :skin_id, :dt)`,
            {
                ':user_id': user_id,
                ':skin_id': skin_id,
                ':dt': unixTime(),
    ...
    calcSkinFileNames(skin_id) {
        const dir =
            UPLOAD_SKIN_DIR + ((skin_id / UPLOAD_SKINS_PER_DIR) | 0) + '/';
    ...
    async upload(data, originalName, type, user_id) {
        ...
        if (existingSkin) {
            ...
            const fileExists = await fs.promises
                .stat(skinFileNames.fullFileName)
                .then(
                    () => true,
                    () => false,
                );
            ...
            if (!result.changes) {
                const row = await this.conn.get(
                    'SELECT id FROM skin WHERE hash = ?',
                    [hash],
                );
                if (!row) {
            ...
            await this.conn.run('UPDATE skin SET file = ? WHERE id = ?', [
                skinFileNames.file,
                skin_id,
            ]);
    ...
    async getOwned(user_id) {
        return await this.conn.all(
            'SELECT skin_id id, file, type FROM user_skin INER JOIN skin ON skin_id = skin.id WHERE user_id = ?',
            [user_id],
        );
    }
    ...
    async deleteFromUser(user_id, skin_id) {
        await this.conn.run(
            'DELETE FROM user_skin WHERE user_id = ? AND skin_id = ?',
            [user_id, skin_id],
        );
    }
    ...
    async getUserSkin(user_id, skin_id) {
        ...
        let row = await this.conn.get(
            'SELECT id, file, type FROM user_skin INER JOIN skin ON skin_id = skin.id WHERE user_id = ? AND skin_id = ?',
            [user_id, skin_id],
        );
