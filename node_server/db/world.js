...
export class DBWorld {
     ...
     */
    async init() {
        this.migrations = new DBWorldMigration(
            this.conn,
            this.world,
            this.getDefaultPlayerStats,
            this.getDefaultPlayerIndicators,
        );
        await this.migrations.apply();
        this.mobs = new DBWorldMob(
            this.conn,
            this.world,
            this.getDefaultPlayerStats,
            this.getDefaultPlayerIndicators,
        ...
        this.bulkLoadDropItemsQuery = new BulkSelectQuery(
            this.conn,
    ...
    async getWorld(world_guid) {
        const row = await this.conn.get('SELECT * FROM world WHERE guid = ?', [
            world_guid,
        ]);
        if (row) {
            const resp = {
                ...
                world_type_id:
                    row.title == config.building_schemas_world_name
                        ? WORLD_TYPE_BUILDING_SCHEMAS
                        : WORLD_TYPE_NORMAL,
                recovery: row.recovery,
            };
            resp.generator = WorldGenerators.validateAndFixOptions(
                resp.generator,
            );
            return resp;
        }
        ...
        await this.conn.run(
            'INSERT INTO world(dt, guid, user_id, title, seed, generator, pos_spawn, game_mode, ore_seed) VALUES (:dt, :guid, :user_id, :title, :seed, :generator, :pos_spawn, :game_mode, :ore_seed)',
            {
                ':dt': unixTime(),
                ':guid': world.guid,
    ...
    async updateAddTime(world_guid, add_time) {
        await this.conn.run(
            'UPDATE world SET add_time = :add_time WHERE guid = :world_guid',
            {
    ...
    async setWorldGameMode(world_guid, game_mode) {
        await this.conn.run(
            'UPDATE world SET game_mode = :game_mode WHERE guid = :guid',
            {
                ':game_mode': game_mode,
                ':guid': world_guid,
            },
    ...
    async saveRecoveryBlob(blob) {
        return this.conn.run('UPDATE world SET recovery = ? WHERE id = ?', [
            blob,
            this.world.info.id,
        ]);
    }
    ...
    async compressModifiers() {
        ...
        const rows = await this.conn.all(
            'SELECT _rowid_ AS rowid, data FROM world_modify_chunks WHERE has_data_blob = 0',
            {},
        );
        const all = [];
        const processChunk = async (rowid, priv, pub, compress_time) => {
            return new Promise(async (resolve, reject) => {
                ...
                const result = await this.conn.run(
                    'UPDATE world_modify_chunks SET data_blob = :data_blob, private_data_blob = :private_data_blob, has_data_blob = 1 WHERE _rowid_ = :_rowid_',
                    {
                        ':data_blob': pub,
                ...
                console.log(
                    `compressModifiers: upd times: compress: ${p1}, store: ${p2} ms`,
                );
                resolve(result);
            });
        };
        for (let row of rows) {
            chunks_count++;
            const p = performance.now();
            const compressed = compressWorldModifyChunk(
                JSON.parse(row.data),
                true,
            );
            // save compressed
        ...
        console.log(
            `compressModifiers: chunks: ${chunks_count}, elapsed: ${
                Math.round((performance.now() - p_start) * 1000) / 1000
            } ms`,
        );
        return true;
    }
    ...
    async registerPlayer(world, player) {
        ...
        const row = await this.conn.get(
            'SELECT id, inventory, pos, pos_spawn, rotate, indicators, chunk_render_dist, game_mode, stats FROM user WHERE guid = ?',
            [player.session.user_guid],
        );
        if (row) {
            const fixInventory = (inventory) => {
                if (inventory.items.length < INVENTORY_SLOT_COUNT) {
                    inventory.items.push(
                        ...new Array(
                            INVENTORY_SLOT_COUNT - inventory.items.length,
                        ).fill(null),
                    );
                }
                // fix list of items
            ...
            return {
                state: state,
                inventory: inventory,
                status: state.indicators.live.value
                    ? PLAYER_STATUS_ALIVE
                    : PLAYER_STATUS_DEAD,
            };
        }
        ...
        // Insert to DB
        await this.conn.run(
            'INSERT INTO user(id, guid, username, dt, pos, pos_spawn, rotate, inventory, indicators, is_admin, stats) VALUES(:id, :guid, :username, :dt, :pos, :pos_spawn, :rotate, :inventory, :indicators, :is_admin, :stats)',
            {
                ...
                ':is_admin':
                    world.info.user_id == player.session.user_id ? 1 : 0,
    ...
    async insertChatMessage(player, params) {
        await this.conn.run(
            'INSERT INTO chat_message(user_id, dt, text, world_id, user_session_id) VALUES (:user_id, :dt, :text, :world_id, :user_session_id)',
            {
    ...
    async bulkUpdatePlayerState(rows, dt) {
        ...
        `);
        return rows.length
            ? run(this.conn, UPDATE.BULK_PLAYER_STATE, {
    ...
    async changePosSpawn(player, params) {
        await this.conn.run(
            'UPDATE user SET pos_spawn = :pos_spawn WHERE id = :id',
            {
                ':id': player.session.user_id,
    ...
    async changeRenderDist(player, value) {
        await this.conn.run(
            'UPDATE user SET chunk_render_dist = :chunk_render_dist WHERE id = :id',
            {
                ':id': player.session.user_id,
    ...
    async loadAdminList(world_id) {
        const resp = [];
        const rows = await this.conn.all(
            'SELECT username FROM user WHERE is_admin = ?',
            [world_id],
        );
        for (let row of rows) {
    ...
    async setAdmin(world_id, user_id, is_admin) {
        await this.conn.get('UPDATE user SET is_admin = ? WHERE id = ?', [
            is_admin,
            user_id,
        ]);
    }
    ...
    async bulkInsertDropItems(rows, dt) {
        ...
        `);
        return rows.length
            ? run(this.conn, INSERT.BULK_DROP_ITEMS, {
                  ':jsonRows': JSON.stringify(rows),
                  ':dt': dt,
              })
            : null;
    ...
    async bulkUpdateDropItems(rows) {
        ...
        `);
        return rows.length
            ? run(this.conn, UPDATE.BULK_DROP_ITEMS, {
                  ':jsonRows': JSON.stringify(rows),
              })
            : null;
    }
    ...
    async changeGameMode(player, game_mode) {
        return await this.conn.run(
            'UPDATE user SET game_mode = :game_mode WHERE id = :id',
            {
                ':id': player.session.user_id,
                ':game_mode': game_mode,
            },
    ...
    async getListTeleportPoints(id) {
        const rows = await this.conn.all(
            'SELECT title, x, y, z FROM teleport_points WHERE user_id = :id ',
            {
                ':id': parseInt(id),
            },
        );
    ...
    async getTeleportPoint(id, title) {
        ...
        const row = await this.conn.get(
            'SELECT x, y, z FROM teleport_points WHERE user_id = :id AND title=:title ',
            {
                ':id': parseInt(id),
                ':title': clear_title,
            },
    ...
    async addTeleportPoint(user_id, title, x, y, z) {
        const clear_title = title.replace(/[^a-z0-9\s]/gi, '').substr(0, 50);
        await this.conn.run(
            'INSERT INTO teleport_points(user_id, title, x, y, z) VALUES (:user_id, :title, :x, :y, :z)',
            {
    ...
    async saveGameRules(world_guid, rules) {
        await this.conn.run(
            'UPDATE world SET rules = :rules WHERE guid = :world_guid',
            {
                ':world_guid': world_guid,
    ...
    async setWorldSpawn(world_guid, pos_spawn) {
        await this.conn.run(
            'UPDATE world SET pos_spawn = :pos_spawn WHERE guid = :world_guid',
            {
                ':world_guid': world_guid,
    ...
    async saveEnderChest(player, ender_chest) {
        await run(
            this.conn,
            'UPDATE user SET ender_chest = :ender_chest WHERE id = :id',
            {
                ':id': player.session.user_id,
    ...
    async loadEnderChest(player) {
        const rows = await this.conn.all(
            'SELECT ender_chest FROM user WHERE id = :id',
            {
                ':id': player.session.user_id,
            },
        );
    ...
    async flushWorld() {
        ...
        try {
            ...
            this.conn.run('UPDATE world SET dt = :dt, add_time = :add_time', {
                ':dt': unixTime(),
                ':add_time': 12000,
            });
