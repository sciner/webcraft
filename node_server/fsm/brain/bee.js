...
export class Brain extends FSMBrain {
    ...
    getFlightBlocks(ignore_nest) {
        ...
        if (legs && legs.id != 0 && legs.material.style == 'default') {
            if (
                ignore_nest ||
                (legs && legs.hasTag && !legs.hasTag('bee_nest'))
            ) {
    ...
    doReturnToHome(delta) {
        ...
        if (Math.random() < 0.02) {
            ...
            mob.rotate.z =
                Math.round(
                    ((mob.rotate.z + (Math.random() * Math.PI) / 4) % 6.28) *
                        1000,
                ) / 1000;
        }
    ...
    doFollow(delta) {
        ...
        if (this.ticks_anger <= this.anger_time) {
            if (
                Math.abs(player.state.pos.y + 2 - mob.pos.y) < 0.5 &&
                this.ticks_attack > this.interval_attack &&
                distance < this.distance_attack
            ) {
                this.ticks_attack = 0;
                switch (difficulty) {
                    ...
                    case EnumDifficulty.NORMAL: {
                        this.target.effects.addEffects([
                            { id: Effect.POISON, level: 1, time: 10 },
                        ]);
                        this.target.setDamage(2);
                        break;
                    }
                    case EnumDifficulty.HARD: {
                        this.target.effects.addEffects([
                            { id: Effect.POISON, level: 2, time: 18 },
                        ]);
