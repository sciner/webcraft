...
export class Brain extends FSMBrain {
    ...
    mobDetonation(rad) {
        ...
        const difficulty = world.rules.getValue('difficulty');
        const mobPosCenter = mobPos.addScalarSelf(
            mob.width / 2,
            mob.height / 2,
            mob.width / 2,
        );
        ...
        // Add sound
        actions.addPlaySound({
            tag: 'madcraft:block.creeper',
            action: 'explode',
            pos: mobPosCenter.clone(),
        });
    ...
    onKill(actor, type_damage) {
        ...
        if (rnd_count_flesh > 0) {
            ...
            actions.addDropItem({
                pos: mob.pos,
                items: [{ id: drop_block.id, count: rnd_count_flesh }],
            });
            actions.addPlaySound({
                tag: 'madcraft:block.creeper',
                action: 'death',
