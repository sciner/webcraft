...
export class Compiler {
    constructor(options) {
        ...
        for (let i = 0; i < options.texture_pack_dir.length; i++) {
            options.texture_pack_dir[i] =
                options.texture_pack_dir[i] + '/assets/minecraft/textures';
        }
        this.options = options;
        // Make default n texture
        this.default_n = new skiaCanvas.Canvas(
            options.resolution,
            options.resolution,
        );
    ...
    getSpritesheet(id) {
        ...
        if (!spritesheet) {
            const tx_cnt =
                this.base_conf.textures[id]?.tx_cnt ?? DEFAULT_TX_CNT;
    ...
    async init() {
        ...
        //
        this.base_conf = (
            await import(this.options.base_conf, {
                assert: { type: 'json' },
            })
        ).default;
    }
    ...
    async run() {
        ...
        for (let texture of this.compile_data.predefined_textures) {
            ...
            const tex = await spritesheet.loadTex(
                texture.image,
                DEFAULT_TEXTURE_SUFFIXES,
            );
            const { sx, sy } = await spritesheet.drawTexture(
                tex.texture,
                texture.x,
                texture.y,
                texture.has_mask,
        ...
        try {
            this.compile_data.blocks = await this.compileBlocks(
                this.compile_data.blocks,
            );
            await this.export();
        } catch (e) {
            console.error(e);
    ...
    async compileBlocks(blocks, spritesheet_storage) {
        ...
        for (let block of blocks) {
            ...
            if (
                ['stairs'].indexOf(block.style_name) >= 0 ||
                block.layering?.slab
            ) {
            ...
            if ('texture' in block) {
                ...
                const spritesheet =
                    spritesheet_storage.getSpritesheet(spritesheet_id);

                const opTextures = async (obj, texture_key) => {
                    if (
                        typeof obj[texture_key] === 'string' ||
                        obj[texture_key] instanceof String
                    ) {
                    ...
                    for (let tid in obj[texture_key]) {
                        ...
                        if (value.indexOf('|') >= 0) {
                            ...
                            tex = {
                                pos: {
                                    x: parseFloat(pos_arr[0]),
                                    y: parseFloat(pos_arr[1]),
                                },
                            };
                        }
                        let x_size = 1;
                        let y_size = 1;
                        const has_mask =
                            tags.includes('mask_biome') ||
                            tags.includes('mask_color');
                        const compile = block.compile;
                        if (!tex) {
                            const img = await spritesheet.loadTex(
                                value,
                                DEFAULT_TEXTURE_SUFFIXES,
                            );
                            //
                            if (img.texture.width == 16) {
                                const scale = 2;
                                const temp = new skiaCanvas.Canvas(
                                    img.texture.width * scale,
                                    img.texture.height * scale,
                                );
                            ...
                            x_size = Math.min(
                                Math.ceil(
                                    img.texture.width / spritesheet.tx_sz,
                                ),
                                spritesheet.tx_cnt,
                            );
                            y_size = Math.min(
                                Math.ceil(
                                    img.texture.height / spritesheet.tx_sz,
                                ),
                                spritesheet.tx_cnt,
                            );
                            //
                            ...
                            //
                            const pos = spritesheet.findPlace(
                                block,
                                x_size,
                                y_size,
                            );
                            ...
                            if (
                                block.name == BLOCK_NAMES.GRASS_BLOCK &&
                                tid == 'side'
                            ) {
                                spritesheet.drawTexture(
                                    dirt_image,
                                ...
                                spritesheet.ctx.globalCompositeOperation =
                                    'source-over';
                                ...
                                for (let i = 0; i < tex.img.width; i++) {
                                    for (let j = 0; j < tex.img.height; j++) {
                                        ...
                                        if (pix[3] == 0) {
                                            const x =
                                                tex.pos.x * spritesheet.tx_sz +
                                                i;
                                            const y =
                                                tex.pos.y * spritesheet.tx_sz +
                                                j;
                                            spritesheet.ctx.fillRect(
                                                x,
                                                y,
                                                1,
                                                1,
                                ...
                                const img_glow = (
                                    await spritesheet.loadTex(
                                        'block/spawner_glow.png',
                                        DEFAULT_TEXTURE_SUFFIXES,
                                    )
                                ).texture;
                                spritesheet.drawTexture(
                                ...
                                await spritesheet.drawTexture(
                                    ...
                                    has_mask,
                                    null,
                                    has_mask ? compile?.overlay_mask : null,
                                    null,
                                    compile,
                                );
                        ...
                        if (block.texture_animations && tex.img) {
                            if (tid in block.texture_animations) {
                                if (block.texture_animations[tid] === null) {
                                    block.texture_animations[tid] = Math.min(
                                        tex.img.height / spritesheet.tx_sz,
                                        spritesheet.tx_cnt,
                                    );
                                }
                            }
                        ...
                        if (compile) {
                            ...
                            if (compile.layers) {
                                for (let layer of compile.layers) {
                                    const layer_img = await spritesheet.loadTex(
                                        layer.image,
                                        DEFAULT_TEXTURE_SUFFIXES,
                                    );
                                    ctx.drawImage(
                        ...
                        const tex_slot_count = has_mask
                            ? tex.x_size / 2
                            : tex.x_size;
                        if (tex_slot_count > 1) {
                            if (obj[texture_key][tid].length == 2)
                                obj[texture_key][tid].push(0);
                ...
                if (block?.inventory?.texture) {
                    ...
                    //
                    const spritesheet =
                        spritesheet_storage.getSpritesheetByID(spritesheet_id);
                    if (value.indexOf('|') >= 0) {
                        const pos_arr = value.split('|');
                        tex = {
                            pos: {
                ...
                if (block?.stage_textures) {
                    const spritesheet =
                        spritesheet_storage.getSpritesheet('default');
                ...
                if (block.redstone?.textures) {
                    ...
                    for (let k of ['dot', 'line']) {
                        for (let i in block.redstone.textures[k]) {
                            ...
                            if (!tex) {
                                tex = {
                                    pos: spritesheet.findPlace(block, 2, 1),
                                };
                                spritesheet.drawTexture(
                                    img.texture,
                                    tex.pos.x,
    ...
    setFlammable(block_name, catch_chance_modifier, destroy_chance_modifier) {
        this.flammable_blocks.set(block_name, {
