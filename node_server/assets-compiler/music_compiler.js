...
export class Music_Compiler {
    async run() {
        try {
            ...
            const musicChanged = await this.syncDirectory(
                SERVER_MUSIC_SOURCE_DIR,
                SERVER_MUSIC_DIR,
                (name) => !name.startsWith('.'),
            );
            console.log(
                musicChanged
                    ? 'The music directory has been updated'
    ...
    async syncDirectory(srcDir, dstDir, filter = (fileName) => true) {
        ...
        if (!fs.existsSync(srcDir)) {
            ...
            fs.writeFileSync(
                `${dstDir}/music.json`,
                JSON.stringify(EMPTY_MUSIC_JSON),
            );
            return false;
        }
        ...
        const srcFilesByName = ArrayHelpers.toObject(
            srcFiles,
            (i, v) => v.name,
        );
        const dstFilesByName = ArrayHelpers.toObject(
            dstFiles,
            (i, v) => v.name,
        );
