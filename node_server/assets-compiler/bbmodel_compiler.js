...
export class BBModel_Compiler extends BBModel_Compiler_Base {
    ...
    createSpritesheet(tx_cnt, resolution, options, id) {
        id =
            id ?? 'bbmodel_texture_' + new String(this.spritesheets.length + 1);
    ...
    async init() {
        this.conf = (
            await import(this.options.conf, {
                assert: { type: 'json' },
            })
        ).default;
        //
    ...
    async run(compiler) {
        ...
        for (let bbmodel of this.conf.bbmodels) {
            ...
            if ('textures' in model_json) {
                const { spritesheet, places } = await this.prepareModel(
                    model_json,
                    id,
                    this.options,
                );
            ...
            console.log(
                `BBModel ... ${id} ${model_json.elements.length} elements (${model_json.polygons} polygons)`,
            );
        ...
        // Compile blocks
        fs.writeFileSync(
            `${this.options.output_dir}/blocks.json`,
            JSON.stringify(await compiler.compileBlocks(blocks, this), null, 4),
        );
        ...
        // Export conf.json
        fs.writeFileSync(
            `${this.options.output_dir}/conf.json`,
