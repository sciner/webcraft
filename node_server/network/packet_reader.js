...
export class PacketReader {
    constructor() {
        ...
        for (let filename of config.clientpackets) {
            import(`./clientpackets/${filename}.js`).then((module) => {
                this.registered_readers.set(
                    module.default.command,
                    module.default,
                );
                console.debug(
                    `Registered client packet reader: ${module.default.command}`,
                );
            });
        }
    }
    ...
    async read(player, packet) {
        if (
            ...
            ![
                ServerClient.CMD_RESURRECTION,
                ServerClient.CMD_CHUNK_LOAD,
            ].includes(packet.name)
        ) {
            return;
        }
        ...
        try {
            ...
            if (reader) {
                ...
                console.log(
                    `ERROR: Not found packet reader for command: ${packet.name}`,
                );
            }
        } catch (e) {
