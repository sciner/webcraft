...
export default class packet_reader {
    ...
    static async read(player, packet) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (
                dist > MAX_DIST_FOR_PICKUP * 2 ||
                !player.game_mode.canPickupItems()
            ) {
                console.error(
                    `ERROR: pickup item so far from player ${dist}m > ${MAX_DIST_FOR_PICKUP}`,
                );
            ...
            if (restored_items.length == items.length) {
                console.log(
                    `restore if not applyed no one item ${restored_items.length} == ${items.length}`,
                );
                this.restoreDropItemForPlayer(player, drop_item);
                continue;
            }
                ...
                let packets_sound = [
                    {
                        ...
                        data: {
                            tag: 'madcraft:entity.item.pickup',
                            action: 'hit',
                        },
                    },
                ];
