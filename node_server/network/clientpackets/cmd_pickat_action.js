...
export default class packet_reader {
    ...
    static async read(player, packet) {
        ...
        if (packet.data.interactMobID || packet.data.interactPlayerID) {
            player.onAttackEntity(
                packet.data.button_id,
                packet.data.interactMobID,
                packet.data.interactPlayerID,
            );
        } else {
            ...
            const actions = await doBlockAction(
                packet.data,
                world,
                player_info,
            ...
            if (packet.data.actions?.blocks?.list) {
                const player_json = JSON.stringify(
                    packet.data.actions.blocks.list,
                );
                ...
                if (!same_results || !correct_destroy) {
                    ...
                    console.error(
                        `player patch blocks '${player.session.username}'`,
                    );
                }
            }
            if (correct_destroy) {
