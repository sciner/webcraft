...
export class WorldGenerators {
    ...
    static validateAndFixOptions(params) {
        ...
        const resp = {
            ...
            options: {},
            rules: {},
            cluster_size: new Vector(
                generator.cluster_size ?? WorldGenerators.list[0].cluster_size,
            ),
        };
        // pos spawn
        ...
        for (let name in generator.options) {
            ...
            resp.options[name] = WorldGenerators.strictType(
        ...
        if ('options' in params) {
            for (let name in params.options) {
                ...
                const option = generator.options[name];
                const value = WorldGenerators.strictType(
                    params.options[name],
                    option.strict_type,
                );
                //
