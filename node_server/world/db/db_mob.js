...
export class DBWorldMob {
    ...
    async initChunksWithMobs() {
        ...
        this._mobs = new Map();
        let rows = await this.db.all(`SELECT entity_id, x, y, z, is_active, is_dead /* DISTINCT
    ...
    async save(mob) {
        ...
        await this.db.run('UPDATE entity SET x = :x, y = :y, z = :z, indicators = :indicators, is_active = :is_active, is_dead = :is_dead, extra_data = :extra_data WHERE entity_id = :entity_id', {
            ':x': mob.pos.x,
            ':y': mob.pos.y,
            ':z': mob.pos.z,
