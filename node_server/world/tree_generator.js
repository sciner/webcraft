...
export class TreeGenerator extends Default_Terrain_Generator {
    ...
    static async getInstance() {
        ...
        // Import trees
        await import('../../www/js/terrain_generator/biomes.js').then(
            (module) => {
    ...
    async generateTree(world, world_chunk, pos, m) {
                ...
                for (let x = -2; x <= 2; x++) {
                    for (let z = -2; z <= 2; z++) {
                        if (!(x == 0 && y == 0 && z == 0)) {
                            ...
                            if (
                                near_block.id > 0 &&
                                !near_block.material.can_replace_by_tree
                            ) {
        ...
        this.setBlock = function (
            ...
            if (
                tblock.id == 0 ||
                tblock.material.is_leaves ||
                tblock.material.is_sapling ||
                tblock.material.material.id == 'plant'
            ) {
                ubl.set(new Vector(x, y, z), {
                    id: block_type.id,
                    extra_data,
                    rotate,
                });
