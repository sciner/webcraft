...
export class MobGenerator {
    ...
    async generate() {
        // Auto generate mobs
        const auto_generate_mobs = this.chunk.world.getGeneratorOptions(
            'auto_generate_mobs',
            true,
        );
        if (auto_generate_mobs) {
            ...
            this.can_generate = this.random.double() < 0.05;
            ...
            if (this.chunk.tblocks) {
                ...
                const blockIter = this.chunk.tblocks.createUnsafeIterator(
                    new TBlock(null, new Vector(0, 0, 0)),
                );
                ...
                if (vc.size > (CHUNK_SIZE_X * CHUNK_SIZE_Z) / 2) {
                    ...
                    for (let [vec, y] of vc.entries()) {
                        if (cnt++ % 2 == 0) {
                            ...
                            if (
                                ...
                                const pos = new Vector(0.5, y + 1, 0.5);
                    ...
                    if (poses.length > 0) {
                        poses.sort(() => 0.5 - Math.random());
                        const index = Math.floor(
                            this.random.double() * this.types.length,
                        );
