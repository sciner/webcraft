...
export class ServerWorld {
    ...
    async initServer(world_guid, db_world, new_title, game) {
        ...
        const newTitlePromise = new_title
            ? db_world.setTitle(new_title)
            : Promise.resolve();
        ...
        this.network_stat = {
            ...
            in_count: 0,
            out_count: 0,
            out_count_by_type: null,
            in_count_by_type: null,
            out_size_by_type: null,
            in_size_by_type: null,
        };
        ...
        console.log(
            `Restored ${this.worldChunkFlags.size} chunks, ${
                this.db.mobs._addrByMobId.size
            } mobs, elapsed: ${(performance.now() - t) | 0} ms`,
        );
    ...
    async makeBuildingsWorld() {
        ...
        for (let schema of BuildingTemplate.schemas.values()) {
            ...
            addBlock(new Vector(schema.world.pos1.x, 1, sign_z), {
                id: 645,
                extra_data: {
                    text: schema.name,
                    username: this.info.title,
                    dt: new Date().toISOString(),
                },
                rotate: new Vector(sign_rot_x, 1, 0),
            });
            // clear basement level
            for (
                let x = 0;
                x <= schema.world.pos1.x - schema.world.pos2.x;
                x++
            ) {
                for (
                    let z = 0;
                    z <= schema.world.pos1.z - schema.world.pos2.z;
                    z++
                ) {
                    const pos = new Vector(
                        schema.world.pos1.x - x,
                        0,
                        schema.world.pos1.z - z,
                    );
                    if (
                        !pos.equal(schema.world.pos1) &&
                        !pos.equal(schema.world.pos2)
                    ) {
        ...
        console.log(
            'Building: store modifiers in db ...',
            performance.now() - t,
        );

        // compress chunks in db
        t = performance.now();
        await this.db.chunks.insertRebuildModifiers(
            Array.from(chunks_addr.keys()),
        );
        console.log(
            'Building: compress chunks in db ...',
            performance.now() - t,
        );
    ...
    autoSpawnHostileMobs() {
        ...
        const auto_generate_mobs = this.getGeneratorOptions(
            'auto_generate_mobs',
            true,
        );
        ...
        if (
            !auto_generate_mobs ||
            !good_world_for_spawn ||
            !this.rules.getValue('doMobSpawning')
        ) {
            return;
        }
        // находим игроков
        for (const player of this.players.values()) {
            if (
                ...
                if (mobs.length <= 4) {
                    ...
                    const x =
                        player.state.pos.x +
                        SPAWN_DISTANCE * (Math.random() - Math.random());
                    ...
                    if (
                        under &&
                        (under.id != 0 ||
                            under.material.style_name == 'planting')
                    ) {
    ...
    updateWorldWeather() {
        ...
        if (
            !this.rules.getValue('doWeatherCycle') ||
            time < this.weather_update_time
        ) {
            return;
        }
        if (this.weather == Weather.CLEAR) {
            this.weather_update_time =
                (Math.random() * MAX_TIME_RAIN) | (0 + MIN_TIME_RAIN + time);
            if (Math.random() < 0.2) {
                this.setWeather(Weather.RAIN);
            }
        } else {
            this.weather_update_time =
                (Math.random() * MAX_TIME_WITHOUT_RAIN) |
                (0 + MIN_TIME_WITHOUT_RAIN + time);
    ...
    updateWorldCalendar() {
        ...
        this.info.calendar.day_time = Math.round(
            (age - this.info.calendar.age) * GAME_DAY_SECONDS,
        );
    }
    ...
    async tick() {
        ...
        if (!this.pause_ticks) {
            ...
            if (this.ticks_stat.number % 100 == 0) {
                ...
                !this.givePriorityToSavingFluids &&
    ...
    async onPlayer(player, skin) {
        ...
        if (existing_player) {
            console.log(
                `OnPlayer delete previous connection for: ${player.session.username}, token=${rndToken}`,
            );
            timer.start('onLeave');
        ...
        this.chat.sendSystemChatMessageToSelectedPlayers(
            `player_connected|${player.session.username}`,
        ...
        player.sendPackets([
            {
                name: ServerClient.CMD_CONNECTED,
                data: {
        ...
        if (this.isBuildingWorld()) {
            player.sendPackets([
                player.effects.addEffects(
                    [{ id: Effect.NIGHT_VISION, level: 1, time: 8 * 3600 }],
                    true,
                ),
            ]);
        }
        if (timer.sum() > 50) {
            ...
            this.chat.sendSystemChatMessageToSelectedPlayers(
                '!langTimes in onPlayer(), ms: ' + values,
                player,
            );
        }
    ...
    sendSelected(packets, selected_players, except_players = null) {
        if (selected_players.sendPackets) {
            // fast check if it's a ServerPlayer
    ...
    onBlockExtraDataModified(tblock, pos = tblock.posworld.clone()) {
        ...
        const data = { pos, item };
        tblock.chunk.dbActor.markBlockDirty(
            data,
            tblock.index,
            BLOCK_DIRTY.UPDATE_EXTRA_DATA,
        );
    ...
    async applyActions(server_player, actions) {
        ...
        if (actions.decrement) {
            server_player.inventory.decrement(
                actions.decrement,
                actions.ignore_creative_game_mode,
            );
        }
        // Decrement (extended)
        if (actions.decrement_extended) {
            server_player.inventory.decrementExtended(
        ...
        if (actions.generate_tree.length > 0) {
            for (let i = 0; i < actions.generate_tree.length; i++) {
                ...
                const chunk = this.chunks.get(
                    getChunkAddr(params.pos.x, params.pos.y, params.pos.z),
                );
                if (chunk) {
                    const new_tree_blocks = await treeGenerator.generateTree(
                        this,
                        chunk,
                        params.pos,
                        params.block,
        ...
        if (actions.blocks?.list) {
            ...
            const ignore_check_air =
                actions.blocks.options &&
                'ignore_check_air' in actions.blocks.options
                    ? !!actions.blocks.options.ignore_check_air
                    : false;
            const on_block_set =
                actions.blocks.options &&
                'on_block_set' in actions.blocks.options
                    ? !!actions.blocks.options.on_block_set
                    : true;
            try {
                const block_pos_in_chunk = new Vector(
                    Infinity,
                    Infinity,
                    Infinity,
                );
                const chunk_addr = new Vector(0, 0, 0);
                const prev_chunk_addr = new Vector(
                    Infinity,
                    Infinity,
                    Infinity,
                );
                ...
                for (let params of actions.blocks.list) {
                    ...
                    if (!(params.item instanceof DBItemBlock)) {
                        ...
                        if (globalThis.asdfasdf++ % 1000 == 0)
                            console.log(globalThis.asdfasdf);
                        params.item = this.block_manager.convertBlockToDBItem(
                            params.item,
                        );
                    }
                    //
                    ...
                    if (isReady) {
                        ...
                        const sanitizedItem = shallowCloneAndSanitizeIfPrivate(
                            params.item,
                        );
                        if (sanitizedItem) {
                        ...
                        if (!ignore_check_air) {
                            if (
                                params.action_id ==
                                ServerClient.BLOCK_ACTION_DESTROY
                            ) {
                                if (params.destroy_block.id > 0) {
                                    const except_players = [];
                                    if (server_player)
                                        except_players.push(server_player);
                        ...
                        if (on_block_set) {
                            const listeners =
                                this.blockListeners.beforeBlockChangeListeners[
                                    oldId
                                ];
                            if (listeners) {
                                for (let listener of listeners) {
                                    const newMaterial =
                                        bm.BLOCK_BY_ID[params.item.id];
                                    var res = listener.onBeforeBlockChange(
                                        chunk,
                                        tblock,
                                        newMaterial,
                                        true,
                                    ...
                                    if (typeof res === 'number') {
                                        chunk.addDelayedCall(
                                            listener.onBeforeBlockChangeCalleeId,
                                            res,
                                            [block_pos],
                                        );
                                    } else {
                        ...
                        if (on_block_set) {
                            ...
                            if (
                                ...
                                chunk.checkDestroyNearUncertainStones(
                                    block_pos.clone(),
                                    params.item,
                                    previous_item,
                                    actions.blocks.options.on_block_set_radius,
                                );
                            }
                            // c.
                            const listeners =
                                this.blockListeners.afterBlockChangeListeners[
                                    tblock.id
                                ];
                            if (listeners) {
                                for (let listener of listeners) {
                                    const oldMaterial = bm.BLOCK_BY_ID[oldId];
                                    const res = listener.onAfterBlockChange(
                                        chunk,
                                        tblock,
                                        oldMaterial,
                                        true,
                                    ...
                                    if (typeof res === 'number') {
                                        chunk.addDelayedCall(
                                            listener.onAfterBlockChangeCalleeId,
                                            res,
                                            [block_pos, oldMaterial.id],
                                        );
                                    } else {
                                        TickerHelpers.pushBlockUpdates(
                                            this.updatedBlocksByListeners,
                        ...
                        if (server_player) {
                            if (
                                ...
                                PlayerEvent.trigger({
                                    ...
                                    data: {
                                        pos: params.pos,
                                        block: params.destroy_block,
                                    },
                                });
                            } else if (
                                params.action_id ==
                                ServerClient.BLOCK_ACTION_CREATE
                            ) {
                                PlayerEvent.trigger({
                        ...
                        if (chunk) {
                            ...
                            postponedActions =
                                postponedActions ??
                                chunk.getOrCreatePendingAction(
                                    server_player,
                                    actions,
                                );
                            postponedActions.addBlock(params);
                        } else {
                            this.dbActor.addChunklessBlockChange(
                                chunk_addr,
                                params,
                            );
                        }
        ...
        if (actions.put_in_backet) {
            ...
            if (
                ...
                currentInventoryItem.id == this.block_manager.BUCKET.id
            ) {
                // replace item in inventory
                inventory.items[inventory.current.index] =
                    actions.put_in_backet;
        ...
        if (actions.put_in_bottle) {
            ...
            if (
                currentInventoryItem &&
                currentInventoryItem.id == this.block_manager.GLASS_BOTTLE.id
            ) {
                // replace item in inventory
                inventory.items[inventory.current.index] =
                    actions.put_in_bottle;
                // send new inventory state to player
                inventory.refresh(true);
            }
        }
        ...
        for (let cp of chunks_packets) {
            if (
                (cp.chunk && cp.packets.length > 0) ||
                cp.custom_packets.length > 0
            ) {
                // send 1
    ...
    getPlayersNear(pos, max_distance, not_in_creative, in_spectator) {
        const world = this;
        const aabb = new AABB()
            .set(pos.x, pos.y, pos.z, pos.x, pos.y, pos.z)
