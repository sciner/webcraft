...
import {
    ITEM_MERGE_RADIUS,
    IMMEDIATELY_DELETE_OLD_DROP_ITEMS_FROM_DB,
...
export class ItemWorld {
    ...
    delete(dropItem, deleteFromDB = true) {
        ...
        if (deleteFromDB) {
            ...
            if (dropItem.dirty === DropItem.DIRTY_DELETE) {
                // if it's never been saved, it'll be DIRTY_CLEAR now
    ...
    #mergeItems(chunk) {
        ...
        while (dropItemI < pendingLength) {
            ...
            while (dropItemJ < this.#mergeableItems.length) {
                ...
                if (
                    indexB < 0 ||
                    Math.floor(dropItemA.pos.distance(dropItemB.pos)) >
                        ITEM_MERGE_RADIUS
                ) {
                    ++dropItemJ;
                    continue;
                ...
                if (
                    dropItemB.items.length === 1 &&
                    dropItemB.items[0].count < dropItemA.items[0].count
                ) {
                    var t = dropItemA;
                    dropItemA = dropItemB;
                    dropItemB = t;
                ...
                if (dropItemA === this.#mergeableItems[dropItemI]) {
                    ...
                    --pendingLength;
                    this.#mergeableItems[dropItemI] =
                        this.#mergeableItems[pendingLength];
                    this.#mergeableItems[pendingLength] =
                        this.#mergeableItems[this.#mergeableItems.length - 1];
                    ...
                    if (dropItemJ < pendingLength) {
                        ...
                        --pendingLength;
                        this.#mergeableItems[dropItemJ] =
                            this.#mergeableItems[pendingLength];
                        this.#mergeableItems[pendingLength] =
                            this.#mergeableItems[
                                this.#mergeableItems.length - 1
                            ];
                    } else {
    ...
    writeToWorldTransaction(underConstruction) {
        ...
        this.world.dbActor.pushPromises(
            this.world.db.bulkDeleteDropItems(
                this.deletedEntityIds,
