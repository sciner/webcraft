        ...
        chat.onCmd(async (player, cmd, args) => {
            if (cmd == '/effect') {
                ...
                if (action == 'give') {
                    ...
                    if (level < 1) {
                        chat.sendSystemChatMessageToSelectedPlayers(`effect_error_level`, [player.session.user_id]);
                        return false;  
                    }
                    if (time < 1) {
                        chat.sendSystemChatMessageToSelectedPlayers(`effect_error_time`, [player.session.user_id]);
                        return false;  
                    }
                    ...
                    if (id == -1) {
                        chat.sendSystemChatMessageToSelectedPlayers(`effect_all_removed`, [player.session.user_id]);
                    } else {
