...
export class SchematicReader {
    ...
    async read(orig_file_name) {
        ...
        if (schematic.blockEntities) {
            for (let i = 0; i < schematic.blockEntities.length; i++) {
                ...
                BlockEntities.set(
                    bePos.set(item.Pos[0], item.Pos[1], item.Pos[2]),
                    item,
                );
            }
        } else {
            console.error(
                'schematic reader not support read chests and other block entities',
            );
        }
        ...
        await schematic.forEach((block, pos) => {
            ...
            if (b) {
                ...
                if (name.indexOf('POTTED_') === 0) {
                    ...
                    if (in_pot_block && in_pot_block.id > 0) {
                        new_block = new DBItemBlock(FLOWER_POT_BLOCK_ID, {
                            item: new DBItemBlock(in_pot_block.id),
                        });
                    }
            ...
            if (!new_block) {
                not_found_blocks.set(
                    name,
                    (not_found_blocks.get(name) ?? 0) + 1,
                );
        ...
        not_found_blocks_arr.sort(function (a, b) {
    ...
    createBlockFromSchematic(block, b) {
        ...
        if (block.entities) {
            if (b.chest) {
                const chest_extra_data = this.parseChestExtraData(
                    block.entities,
                    props,
                );
                if (chest_extra_data) {
                ...
                // glowing
                if (
                    'GlowingText' in block.entities &&
                    block.entities.GlowingText
                ) {
        ...
        if (props) {
                ...
                if (b.tags.includes('rotate_by_pos_n_12')) {
                    if ('face' in props && 'facing' in props) {
                        ...
                        if (props.face == 'ceiling') {
                            new_block.rotate.x = Math.max(
                                facings4.indexOf(props.facing),
                                0,
                            );
            ...
            if ('rotation' in props) {
                ...
                new_block.rotate.x =
                    (((parseInt(props.rotation) + 8) % 16) / 16) * 4;
            }
            //
            if ('open' in props) {
            ...
            if (new_block.rotate) {
                ...
                if ('facing' in props) {
                    if (b.tags.includes('rotate_by_pos_n_6')) {
                        ...
                        if (
                            ['stairs', 'door', 'cocoa', 'anvil'].indexOf(
                                b.style_name,
                            ) >= 0
                        ) {
            ...
            if (b.layering && b.layering.slab && 'type' in props) {
                if (props.type == 'top') {
                    ...
                    const double_block = b.layering.full_block_name
                        ? BLOCK.fromName(b.layering.full_block_name)
                        : b;
            ...
            if (b.name == 'BAMBOO') {
                ...
                if ('vertical_direction' in props) {
                    setExtraData(
                        'dir',
                        props.vertical_direction == 'up' ? 1 : -1,
                    );
                }
            ...
            if (chest_item_name) {
                ...
                if (chest_item_block && chest_item_block.id > 0) {
                    ...
                    if (count > 0) {
                        ...
                        const chest_slot =
                            BLOCK.convertItemToDBItem(chest_item_block);
                    ...
                    const chest_slot = BLOCK.convertItemToDBItem(
                        BLOCK.fromName('TEST'),
                    );
