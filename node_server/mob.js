        ...
        if (this.pos.equal(state.pos)) {
            if (this.rotate.equal(state.rotate)) {
                if (
                    ...
                    JSON.stringify(state.extra_data)
                ) {
                    return true;
                }
            }
...
export class Mob {
    ...
    constructor(world, params, existsInDB) {
        ...
        this.lastSavedTime =
            performance.now() + Math.random() * 0.5 * MOB_SAVE_PERIOD;
    ...
    punch(server_player, params) {
        if (params.button_id == MOUSE.BUTTON_RIGHT) {
            this.#brain.onUse(
                server_player,
                server_player.state.hands.right.id,
            );
    ...
    playerCanBeAtacked(player) {
        return (
            ...
            !player.game_mode.getCurrent().can_take_damage
        );
    }
    ...
    exportState(return_diff = false) {
        ...
        if (return_diff && this.#prev_state) {
            ...
            if (
                JSON.stringify(new_state.extra_data) ==
                JSON.stringify(this.#prev_state.extra_data)
            ) {
    ...
    writeToWorldTransaction(underConstruction, force = false) {
        ...
        if (!this.isAlive()) {
            // dying is important, don't skip this transaction
        ...
        if (
            ...
            this.pos.distanceSqr(this.lastSavedPos) <
                MOB_SAVE_DISTANCE * MOB_SAVE_DISTANCE
        ) {
            return;
        }
        this.lastSavedTime =
