...
export default class Ticker {
    ...
    static func(tick_number, world, chunk, v) {
        ...
        if (state.level != 0) {
            ...
            if (
                (state.level > 0 &&
                    (state.first == Effect.SPEED ||
                        state.first == Effect.HASTE)) ||
                (state.level > 1 &&
                    (state.first == Effect.RESISTANCE ||
                        state.first == Effect.JUMP_BOOST)) ||
                (state.level > 2 && state.first == Effect.STRENGTH)
            ) {
                effect = state.first;
            }
            ...
            for (const player of players) {
                player.effects.addEffects([
                    { id: effect, time: time, level: level },
                ]);
            }
        }
