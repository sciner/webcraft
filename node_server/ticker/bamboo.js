...
export default class Ticker {
    ...
    static func(tick_number, world, chunk, v, check_pos, ignore_coords) {
        ...
        if (extra_data && extra_data.stage < ticking.max_stage) {
            const mul =
                4 * world.getGeneratorOptions('sapling_speed_multipliyer', 1);
            if (tick_number % (ticking.times_per_stage * mul) == 0) {
                ...
                if (extra_data.stage == 0) {
                    ...
                    const over1 = world.getBlock(
                        v.pos.add(new Vector(0, 1, 0)),
                    );
                    const under1 = world.getBlock(
                        v.pos.add(new Vector(0, -1, 0)),
                    );
                    if (extra_data.stage == 1) {
                        if (
                            !over1 ||
                            over1.id == 0 ||
                            over1.material.material.id == 'leaves'
                        ) {
                            if (
                                under1.id == tblock.id &&
                                (!under1.extra_data ||
                                    under1.extra_data.stage == 3)
                            ) {
                                addNextBamboo(v.pos, tblock, 1);
                            }
                            if (
                        ...
                        if (extra_data.stage == 2 && extra_data.pos) {
                            if (
                                ...
                                if (
                                    ...
                                    if (
                                        over1.posworld.distance(
                                            extra_data.pos,
                                        ) <
                                        extra_data.max_height - 1
                                    ) {
                                        if (
                                            addNextBamboo(
                                                over1.posworld,
