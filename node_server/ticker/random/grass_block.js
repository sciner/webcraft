...
export default function randomTicker(world, actions, world_light, tblock) {
    const over_src_block = world.getBlock(
        _rnd_pos_up.copyFrom(tblock.posworld).addScalarSelf(0, 1, 0),
    );
    if (
        world_light < 4 ||
        (over_src_block && !isLightOpacity(over_src_block))
    ) {
        // трава зачахла
        ...
        if (rnd_block && rnd_block.id == BLOCK.DIRT.id) {
            const over_block = world.getBlock(
                _rnd_pos_up.copyFrom(rnd_block.posworld).addScalarSelf(0, 1, 0),
            );
            if (over_block && isLightOpacity(over_block)) {
                actions.addBlocks([
                    {
                        pos: rnd_block.posworld.clone(),
                        item: { id: tblock.id },
                        action_id: ServerClient.BLOCK_ACTION_REPLACE,
                    },
                ]);
