...
import { ArrayHelpers, ArrayOrMap } from '../../www/js/helpers.js';
import { ServerClient } from '../../www/js/server_client.js';
import {
    FLUID_TYPE_MASK,
    FLUID_WATER_INTERACT,
    FLUID_WATER_REMOVE,
    FLUID_WATER_ABOVE_INTERACT,
    FLUID_WATER_ABOVE_REMOVE,
...
export class BlockListeners {
        ...
        this.beforeBlockChangeListeners = this.#loadList(
            ...
            (listener, calleeId) => {
                ...
                return function (chunk, pos) {
                    const tblock = chunk.getBlock(
                        pos,
                        tmp_DelayedBlockListener_block,
                    );
                    const upd_blocks = listener.onBeforeBlockChange(
                        chunk,
                        tblock,
                        tblock.material,
                        false,
                    ...
                    TickerHelpers.pushBlockUpdates(
                        chunk.blocksUpdatedByListeners,
                        upd_blocks,
                    );
                };
            },
        ...
        this.afterBlockChangeListeners = this.#loadList(
            ...
            (listener, calleeId) => {
                ...
                return function (chunk, pos, oldBlockId) {
                    ...
                    const upd_blocks = listener.onAfterBlockChange(
                        chunk,
                        tblock,
                        BLOCK.BLOCK_BY_ID[oldBlockId],
                        false,
                    );
                    TickerHelpers.pushBlockUpdates(
                        chunk.blocksUpdatedByListeners,
        ...
        this.fluidChangeListeners = this.#loadList(
            ...
            (listener, calleeId) => {
                ...
                return function (chunk, pos) {
                    ...
                    const upd_blocks = listener.onFluidChange(
                        chunk,
                        tblock,
                        tblock.fluid,
                        false,
            ...
            (listener, calleeId) => {
                ...
                return function (chunk, pos) {
                    ...
                    if ((tblock.fluid & FLUID_TYPE_MASK) === 0) {
                        const upd_blocks = listener.onFluidRemove(
                            chunk,
                            tblock,
                            false,
                        );
        ...
        this.fluidAboveChangeListeners = this.#loadList(
            ...
            (listener, calleeId) => {
                ...
                return function (chunk, pos) {
                    ...
                    const upd_blocks = listener.onFluidAboveChange(
                        chunk,
                        tblock,
                        tblock.fluid,
                        false,
        ...
        this.fluidAboveRemoveListeners = this.#loadList(
            ...
            (listener, calleeId) => {
                ...
                return function (chunk, pos) {
                    ...
                    if ((tblock.fluid & FLUID_TYPE_MASK) === 0) {
                        const upd_blocks = listener.onFluidAboveRemove(
                            chunk,
                            tblock,
                            false,
                        );
    ...
    #loadList(conf, calleeIdSuffix, fluidBlockProps, calleeFactoryFn) {
        // parse config
        const result = parseManyToMany(
            conf,
            (name) => BLOCK.fromName(name.toUpperCase()).id,
            [],
        );
        // import code
        const p = loadMappedImports(
            result,
            './ticker/listeners/',
            importClassInstanceWithId,
            this.uniqueImports,
