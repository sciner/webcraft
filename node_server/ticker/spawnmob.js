    ...
    static func(tick_number, world, chunk, v) {
        ...
        if (tick_number % extra_data.max_ticks == 0) {
            ...
            if (extra_data?.limit?.count === 1) {
                const spawn_pos = pos.add(new Vector(0.5, 0, 0.5));
                const params = {
            ...
            const mobs = world.getMobsNear(pos, SPAWN_PLAYER_DISTANCE, [
                extra_data.type,
            ]);
            if (mobs.length > 5) {
            ...
            for (let i = 0; i < SPAWN_ATTEMPTS - mobs.length; i++) {
                const x = Math.floor(
                    Math.random() * (SPAWN_RAD_HOR * 2 + 1) + -SPAWN_RAD_HOR,
                );
                const z = Math.floor(
                    Math.random() * (SPAWN_RAD_HOR * 2 + 1) + -SPAWN_RAD_HOR,
                );
                ...
                for (let player of players) {
                    ...
                    if (
                        check_pos.x == spawn_pos.x &&
                        check_pos.z == spawn_pos.z
                    ) {
                        spawn_disabled = true;
                        break;
                    }
            ...
            updated_blocks.push({
                pos: v.pos.clone(),
                item: { id: tblock.id, extra_data: extra_data },
                action_id: ServerClient.BLOCK_ACTION_MODIFY,
            });
            return updated_blocks;
        }
