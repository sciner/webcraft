...
function getFreePosition(world, pos) {
    ...
    for (const side of sides) {
        ...
        if (
            !body ||
            !under ||
            body.id == BLOCK.MELON.id ||
            body.id == BLOCK.PUMPKIN.id
        ) {
            return;
        }
        if (
            body.id == BLOCK.AIR.id &&
            (under.material.material.id == 'dirt' ||
                under.material.material.id == 'sand')
        ) {
            //dirt grass sand
            facing.push(side);
        }
...
export default class Ticker {
    ...
    static func(tick_number, world, chunk, v) {
        ...
        if (is_bone) {
            extra_data.stage = Math.min(
                extra_data.stage + extra_data.bone,
                ticking.max_stage,
            );
            extra_data.bone = 0;
        }
        const pos = v.pos.clone();
        if (tblock.id == BLOCK.KELP.id) {
            // Эти блоки растут вверх, копируя основание. При срубании, рост продолжен, но в воде
            ...
            if (
                ...
                (block.fluid & FLUID_TYPE_MASK) === FLUID_WATER_ID
            ) {
                return [
                    {
                        pos: pos.offset(0, stage, 0),
                        item: { id: tblock.id, extra_data: { notick: true } },
                        action_id: ServerClient.BLOCK_ACTION_CREATE,
                    },
                ];
            }
        } else if (
            tblock.id == BLOCK.SUGAR_CANE.id ||
            tblock.id == BLOCK.CACTUS.id
        ) {
            // Эти блоки растут вверх, копируя основание. При срубании, рост продолжен
            // проверяем срубили ли кусок
            let stage = 0;
            ...
            tblock.id == BLOCK.MELON_SEEDS.id ||
            tblock.id == BLOCK.PUMPKIN_SEEDS.id
        ) {
            // Эти блоки растут как семена в области одного блока, но по истечению роста дают плоды
        ...
        } else if (tblock.material.tags.includes('sapling')) {
            // Это саженцы, по окончанию роста они превращются в деревья
            if (extra_data.stage == ticking.max_stage) {
                const params = {
                    pos: pos,
            ...
            // Эти блоки растут как семена в области одного блока. По истечению роста, действий больше нет
            if (extra_data.stage == ticking.max_stage) {
