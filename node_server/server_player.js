...
import {
    MAX_PORTAL_SEARCH_DIST,
    PLAYER_MAX_DRAW_DISTANCE,
    PORTAL_USE_INTERVAL,
    MOUSE,
    PLAYER_STATUS_DEAD,
...
export class NetworkMessage {
    constructor({ time = Date.now(), name = '', data = {} }) {
        this.time = time;
        this.name = name;
        this.data = data;
    }
...
class ServerPlayerSharedProps {
    ...
    get isAlive() {
        return this.p.live_level > 0;
    }
    get user_id() {
        return this.p.session.user_id;
    }
    get pos() {
        return this.p.state.pos;
    }
    get sitting() {
        return this.p.state.sitting;
    }
}
...
export class ServerPlayer extends Player {
    ...
    init(init_info) {
        ...
        this.game_mode.onSelect = async (mode) => {
            ...
            this.sendPackets([
                { name: ServerClient.CMD_GAMEMODE_SET, data: mode },
            ]);
            this.world.chat.sendSystemChatMessageToSelectedPlayers(
                `game_mode_changed_to|${mode.title}`,
    ...
    async onJoin(session_id, skin_id, conn, world) {
        ...
        this.skin = await Qubatch.db.skins.getUserSkin(
            session.user_id,
            skin_id,
        );
        //
    ...
    sendWorldInfo(update) {
        this.sendPackets([
            {
                name: update
                    ? ServerClient.CMD_WORLD_UPDATE_INFO
                    : ServerClient.CMD_WORLD_INFO,
                data: this.world.getInfo(),
            },
        ]);
    }
    ...
    async onMessage(message) {
        ...
        try {
            ...
            if (ns.in_count_by_type) {
                ...
                ns.in_count_by_type[name] =
                    (ns.in_count_by_type[name] ?? 0) + 1;
                if (ns.in_size_by_type) {
                    ns.in_size_by_type[name] =
                        (ns.in_size_by_type[name] ?? 0) + message.length;
                }
            }
    ...
    sendPackets(packets) {
        ...
        if (ns.out_count_by_type && packets.length) {
            ...
            if (ns.out_size_by_type) {
                ...
                if (!hasDifferentTpes) {
                    ns.out_size_by_type[name] =
                        (ns.out_size_by_type[name] ?? 0) + json.length;
                }
            }
            // for each packet
            for (const p of packets) {
                const name = p.name;
                ns.out_count_by_type[name] =
                    (ns.out_count_by_type[name] ?? 0) + 1;

                if (ns.out_size_by_type && hasDifferentTpes) {
                    const len = JSON.stringify(p).length + 1;
                    ns.out_size_by_type[name] =
                        (ns.out_size_by_type[name] ?? 0) + len;
                }
            }
        }
    ...
    changePosSpawn(params) {
        ...
        let message =
            'Установлена точка возрождения ' +
            params.pos.x +
            ', ' +
            params.pos.y +
            ', ' +
            params.pos.z;
        this.world.chat.sendSystemChatMessageToSelectedPlayers(message, [
            this.session.user_id,
        ]);
    }
    ...
    updateHands() {
        ...
        // Get materials
        let left_hand_material =
            inventory.current.index2 >= 0
                ? inventory.items[inventory.current.index2]
                : null;
    ...
    getEyePos() {
        ...
        return this._eye_pos.set(
            this.state.pos.x,
            this.state.pos.y + this.height * 0.9375 - subY,
            this.state.pos.z,
        );
    }
    ...
    async checkWaitPortal() {
        ...
        if (wait_info.params?.found_or_generate_portal) {
            ...
            if (from_portal_id) {
                from_portal = await this.world.db.portal.getByID(
                    from_portal_id,
                );
                if (from_portal) {
            ...
            if (from_portal?.pair) {
                exists_portal = await this.world.db.portal.getByID(
                    from_portal?.pair.id,
                );
            }
            if (!exists_portal) {
                exists_portal = await this.world.db.portal.search(
                    wait_info.pos,
                    MAX_PORTAL_SEARCH_DIST,
                );
            }
            ...
            if (exists_portal) {
                ...
                for (let chunk of this.world.chunks.getAround(
                    wait_info.pos,
                    this.state.chunk_render_dist,
                )) {
                    if (chunk.isReady()) {
                        const new_portal =
                            this.world.portals.foundPortalFloorAndBuild(
                                this.session.user_id,
                                chunk,
                                from_portal_type,
                            );
                        if (new_portal) {
                            ...
                            if (from_portal) {
                                ...
                                await this.world.db.portal.setPortalPair(
                                    from_portal.id,
                                    {
                                        id: new_portal.id,
                                        player_pos: new_portal.player_pos,
                                    },
                                );
                                ...
                                await this.world.db.portal.setPortalPair(
                                    ...
                                    {
                                        id: from_portal.id,
                                        player_pos: from_portal.player_pos,
                                    },
                                );
                            }
                            sendTeleport();
    ...
    checkWaitingData() {
        ...
        // teleport
        let initialPos =
            this.vision.safePosInitialOverride || this.state.pos_spawn;
        this.vision.safePosInitialOverride = null;
        this.state.pos = this.world.chunks.findSafePos(
            initialPos,
            this.vision.safeTeleportMargin,
        );
    ...
    checkInPortal() {
        ...
        if (this.prev_use_portal) {
            if (
                performance.now() - this.prev_use_portal <
                PORTAL_USE_INTERVAL
            ) {
                return false;
            }
        ...
        const portal_block = tblock_legs?.material?.is_portal
            ? tblock_legs
            : null;
    ...
    teleport(params) {
        ...
        if (params.pos) {
            ...
            switch (params.place_id) {
                ...
                case 'random': {
                    new_pos = new Vector(
                        Math.random() * MAX_RANDOM_TELEPORT_COORD -
                            Math.random() * MAX_RANDOM_TELEPORT_COORD,
                        120,
                        Math.random() * MAX_RANDOM_TELEPORT_COORD -
                            Math.random() * MAX_RANDOM_TELEPORT_COORD,
            ...
            // teleport by portal
            const portal_type = WorldPortal.getPortalTypeByID(
                params.portal.type,
            );
            if (portal_type) {
        ...
        if (new_pos) {
            ...
            if (
                Math.abs(new_pos.x) > MAX_COORD ||
                Math.abs(new_pos.y) > MAX_COORD ||
                Math.abs(new_pos.z) > MAX_COORD
            ) {
            ...
            if (params.safe) {
                ...
                this.sendPackets([
                    {
                        name: ServerClient.CMD_SET_STATUS_WAITING_DATA,
                        data: {},
                    },
                ]);
                ...
                teleported_player.wait_portal = new WorldPortalWait(
                    teleported_player.state.pos
                        .clone()
                        .addScalarSelf(0, this.height / 2, 0),
                    new_pos,
                    params,
                );
    ...
    isMiningComplete(data) {
        ...
        const mining_time_server =
            block.material.getMiningTime({ material: instrument }, false) / mul;
