...
export var MinimalGLTFLoader = MinimalGLTFLoader || {};
 ...
 */
glTFLoader.prototype.loadGLTF = function(uri, options, callback) {
    ...
    // Make options
    let default_options = {
        baseUri: _getBaseUri(uri)
    };
    options = Object.assign(default_options, options);

    //
    this.baseUri = options.baseUri; // _getBaseUri(uri);
...
function _loadImage(url, iid, onload) {
    if(url.lastIndexOf('data:image/') === 0) {
        var img = new Image();
        img.src = url;
        return onload(img, iid);;
    }
    var img = new Image();
