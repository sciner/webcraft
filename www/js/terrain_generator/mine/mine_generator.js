...
export class MineGenerator {
    ...
    genEnter(chunk, random, dir) {
        ...
        this.setBlock(chunk, vec.x, vec.y, vec.z, BLOCK.TORCH, true); 
    ...
    genSmallHal(chunk, random, dir){
        ...
        for (let n = 0; n <= 15; n += interval) {
            ...
            //факел
            this.genBoxAir(chunk, 3, 3, n - 3, 3, 3, n + 3, dir, BLOCK.TORCH, random, 0.1);
            this.genBoxAir(chunk, 1, 3, n - 3, 1, 3, n + 3, dir, BLOCK.TORCH, random, 0.1);
        }
    }
    ...
    genSmallCross(chunk, random, dir){
        ...
        for (let n = 0; n < 16; n += interval) {
            ...
            //факелы
            this.genBoxAir(chunk, 1, 3, n - 3, 1, 3, n + 3, dir, BLOCK.TORCH, random, 0.2);
            this.genBoxAir(chunk, 3, 3, n - 3, 3, 3, n + 3, dir, BLOCK.COBWEB, random, 0.1);
            
            this.genBoxAir(chunk, n - 3, 3, 14, n + 3, 3, 14, dir, BLOCK.TORCH, random, 0.1);
            this.genBoxAir(chunk, n - 3, 3, 12, n + 3, 3, 12, dir, BLOCK.TORCH, random, 0.2);
        }
    }
     ...
     */
    genBoxAir(chunk, minX, minY, minZ, maxX, maxY, maxZ, dir = DIRECTION_BIT.NORTH, block = {id : 0}, random = null, chance = null) {
        for (let x = minX; x <= maxX; ++x) {
            for (let y = minY; y <= maxY; ++y) {
                for (let z = minZ; z <= maxZ; ++z) {
                    let vec = (new Vector(x, y, z)).rotY(dir);
