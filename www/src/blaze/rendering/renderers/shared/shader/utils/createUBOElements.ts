...
export function createUBOElements(uniformData: UniformData[]): UniformBufferLayout
{
    ...
    for (let i = 0; i < uboElements.length; i++)
    {
        ...
        //        console.log('----->', uboElement);

        if (!size)
        {
            throw new Error(`Unknown type ${uboElement.data.type}`);
            // if (uboElement.data.type.includes('array<vec4<f32>'))
            // {
            //     const extractedLength = parseInt(uboElement.data.type
            //         .replace('array<vec4<f32>,', '')
            //         .replace('>', ''),
            //     10);// (regex)[0];

            //     uboElement.data.type = 'vec4<f32>';
            //     uboElement.data.length = extractedLength;

            //     size = extractedLength * 16;
            // }
        }
