...
export type ShaderDescriptor = ShaderWithGroupsDescriptor | ShaderWithResourcesDescriptor;
...
export class Shader
{
    ...
    uniformBindMap: Record<number, Record<number, any>> = {};
    ...
    constructor({ gpuProgram, glProgram, groups, resources, groupMap }: any /*ShaderDescriptor*/)
    {
        ...
        else if (!gpuProgram && groups && groupMap)
        {
            for (const i in groupMap)
            {
                for (const j in groupMap[i])
                {
                    ...
                    nameHash[uniformName] = { group: +i, binding: +j, name: uniformName };
                }
            }
        }
        else if (gpuProgram && groups && !groupMap)
