...
export class PipelineSystem implements ISystem
{
    ...
    gpu: GPU;
    stencilState: GPUDepthStencilState;
    ...
    createPipeline(geometry: Geometry, program: GpuProgram, state: State, topology: Topology): GPURenderPipeline
    {
        ...
        blendModes[0].writeMask = this.stencilMode === STENCIL_MODES.RENDERING_MASK ? 0 : this.colorMask;
    ...
    getModule(code: string): GPUShaderModule
    {
        return this._moduleCache[code] || this.createModule(code);
    }

    createModule(code: string): GPUShaderModule
    {
        const device = this.gpu.device;
    ...
    generateProgramKey(program: GpuProgram): number
    {
    ...
    generateBufferKey(geometry: Geometry): number
    {
        const keyGen = [];
        let index = 0;
        // generate a key..
    ...
    createVertexBufferLayouts(geometry: Geometry): GPUVertexBufferLayout[]
    {
