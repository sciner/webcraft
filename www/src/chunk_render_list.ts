...
import {GROUPS_NO_TRANSPARENT, GROUPS_TRANSPARENT} from "./renders/shared.js";
...
export class ChunkRenderList {
    ...
    draw(render: Renderer, resource_pack: BaseResourcePack, transparent: boolean) {
        ...
        const player = render.player;
        const hide_creative_mat = player.isHideCreativeMaterialBlocks()

        for (let group of groups) {
            if(hide_creative_mat && group == 'creative_regular') {
                continue
            }
